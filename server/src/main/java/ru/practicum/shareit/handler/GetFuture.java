...
public class GetFuture extends BookingStateHandler {
    ...
    public List<BookingDto> getBookings(Long senderId, UserType userType, PageRequest pageRequest) {
        ...
        if (userType.equals(USER)) {
            bookings = bookingStorage.findAllByBookerIdAndStatusFuture(senderId, LocalDateTime.now(), pageRequest);
        }
        if (userType.equals(OWNER)) {
            bookings = bookingStorage.findAllByItemOwnerIdAndStatusFuture(senderId, LocalDateTime.now(), pageRequest);
        }
